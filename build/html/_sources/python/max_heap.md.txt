...
> 많은 파이썬 이용자들이 `heapq` 표준 라이브러리를 사용하지만 해당 라이브러리는 기본저긍로 __Min Heap__ 구현하도록 설계되어있는 단점이 있습니다.
>
> 그러나 `heapq` 소스코드에는 __Max Heap__ 의 많은 부분이 이미 구현되어있습니다.
> 
> 본 문서에서는 `heapq` 라이브러리의 숨겨진 함수들을 이용해 __Max Heap__ 을 구현하는 방법에 대해 알아보겠습니다.
